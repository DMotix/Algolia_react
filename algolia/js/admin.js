...
function countElementText(elName, maximum) {
    var characterCount = $('#'+elName).val().length,
    current = $('#'+elName+'-count');
    current.text(characterCount);
    if (characterCount >= maximum) {
        $('#'+elName+'-count').css('color', 'red');
        $('#'+elName+'-count').css('font-weight', 'bold');
    } else {
        $('#'+elName+'-count').attr('style', '');
    }
}
...
jQuery(document).ready(function($) {
    ...
    */
    ...
    $('#new-search-term-href').click(function () {
        $('#new-search-term input[type="text"]').val('');
        $('#id-searchterm').val('');
        var img = $('#add-rule-icon').children('img').eq(0);
        img.attr('src', img.data('path') + 'add.gif');
        var span = $('#add-rule-icon').children('span').eq(0);
        span.text('New Search Term');
        $('#new-search-term').slideDown();
    });

    $('#new-search-term-cancel').click(function () {
        $('#new-search-term').slideUp();
    });

    $('.term-edit').click(function () {
        var row = $(this).closest('td').closest('tr');
        $('#new-term-term').val(row.children('td').eq(0).text());
        $('#new-term-url').val(row.children('td').eq(1).data('url'));
        $('#new-term-title').val(row.children('td').eq(2).text());
        $('#new-term-description').val(row.children('td').eq(3).text());
        $('#id-searchterm').val($(this).data('id'));
        var img = $('#add-rule-icon').children('img').eq(0);
        img.attr('src', img.data('path') + 'edit.gif');
        var span = $('#add-rule-icon').children('span').eq(0);
        span.text('Edit Search Term');
        countElementText('new-term-title', 70);
        countElementText('new-term-description', 140);
        $('#new-search-term').slideDown();
    });
    ...
    $('.term-delete').click(function () {
        ...
        if (conf) {
            $('#delete-term-'+delId).submit();
        }
    });

    $('#edit-search-prefix').click(function () {
        $('#algolia-search-prefix').focus();
    });
    $('#algolia-search-prefix-update').click(function () {
        $(this).parent('td').eq(0).append(
            $('<input>')
            .attr({
                type: 'hidden',
                name: 'updateSearchPrefix'
            })
            );
        $('#search_term > form').submit();
    });
    
    $('#new-term-title').keyup(function() {
        countElementText('new-term-title', 70);
    });
    $('#new-term-description').keyup(function() {
        countElementText('new-term-description', 140);
    });
});