	...
	{foreach $option_list AS $category => $categoryData}
		...
		<div class="panel {if isset($categoryData['class'])}{$categoryData['class']}{/if}" id="{$table}_fieldset_{$category}">
			...
			<div class="panel-heading">
				<i class="{if isset($categoryData['icon'])}{$categoryData['icon']}{else}fa fa-gears{/if}"></i>
			...
			<div class="well clearfix">
				<label class="control-label col-lg-3">
					<i class="icon-site-map"></i> {l s='Multistore'}
				</label>
				<div class="col-lg-9">
			...
			{foreach $categoryData['fields'] AS $key => $field}
					...
					{else}
						<div class="form-group{if isset($field.form_group_class)} {$field.form_group_class}{/if}"{if isset($tabs) && isset($field.tab)} data-tab-id="{$field.tab}"{/if}>
                                                    {if isset($field['sub_title']) && $field['sub_title'] != ''}<hr/><p><strong style="text-transform: uppercase;">{$field['sub_title']}</strong></p><hr/>{/if}
							<div id="conf_id_{$key}" class="clearfix{if $field['is_invisible']} isInvisible{/if}">
								{block name="label"}
								...
								{block name="input"}
									...
									{elseif $field['type'] == 'checkbox'}
										<div class="col-lg-9">
                                                                                        {if strpos($field['value'], ",") != false}
                                                                                            {assign var=array_value value=','|explode:$field['value']}
                                                                                        {/if}
											{foreach $field['choices'] AS $k => $v}
												<p class="checkbox clearfix col-lg-4">
													{strip}
													<label for="{$key}{$k}_on">
														<input type="checkbox" name="{$key}[]" id="{$key}{$k}_on" value="{$k|intval}"{if strpos($field['value'], ",") != false}{if $k|in_array:$array_value} checked="checked"{/if}{else}{if $k == $field['value']} checked="checked"{/if}{/if}{if isset($field['js'][$k])} {$field['js'][$k]}{/if}/>
													 	{$v}
													</label>
													{/strip}
												</p>
									...
									{elseif $field['type'] == 'textarea'}
										<div class="col-lg-9">
											<textarea class="textarea-autosize{if isset($field.class)} {$field.class}{/if}" name={$key} cols="{$field['cols']}" rows="{$field['rows']}">{$field['value']|escape:'html':'UTF-8'}</textarea>
										</div>
									...
									{elseif $field['type'] == 'color'}
										<div class="col-lg-2">
											<div class="input-group">
												<input type="color" size="{$field['size']}" data-hex="true" name="{$key}" class="color mColorPickerInput{if isset($field['class'])} {$field['class']}{/if}" value="{if !isset($field['value']) || $field['value'] == ""}{$field['default_value']|escape:'html':'UTF-8'}{else}{$field['value']|escape:'html':'UTF-8'}{/if}" />
											</div>
							            </div>
